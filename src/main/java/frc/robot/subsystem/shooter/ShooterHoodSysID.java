...
public class ShooterHoodSysID {
	...
	public void actuate(Voltage volts) {
		shooter.hw.hoodTalonFX.setVoltage(volts.magnitude());
	}

	public void log(SysIdRoutineLog log) {
		log.motor("hood")
				.voltage(voltage.mut_replace(shooter.inputs.hoodVoltageVolts, Volts))
				.angularPosition(position.mut_replace(shooter.inputs.hoodPosRadians, Radians))
				.angularVelocity(velocity.mut_replace(shooter.inputs.hoodVelRadiansPerSec, RadiansPerSecond));
	}

	public void init() {
		shooter.hw.sysIdHood = true;
